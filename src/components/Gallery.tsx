        ...
        <div ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-label={t('galleryTitle')} className="fixed inset-0 bg-ink/95 z-[60] flex items-center justify-center outline-none" onClick={close}>
          <div className="relative w-full max-w-5xl max-h-full p-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center justify-between mt-4">
              <button onClick={prev} aria-label="Previous image" className="text-paper text-2xl px-4 py-3 hover:text-gold active:text-gold transition-colors">&#8249;</button>
              ...
              <button onClick={next} aria-label="Next image" className="text-paper text-2xl px-4 py-3 hover:text-gold active:text-gold transition-colors">&#8250;</button>
            </div>
          </div>
          <button onClick={close} aria-label="Close" className="absolute top-4 end-4 text-paper text-3xl p-3 min-w-[44px] min-h-[44px] inline-flex items-center justify-center hover:text-gold active:text-gold transition-colors">&#x2715;</button>
        </div>
      )}
    </div>
  );
